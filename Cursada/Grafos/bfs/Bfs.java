...
public class Bfs {
	...
	private Queue<Nodo> nodosVisitados;

	public Bfs(int grado, boolean dirigido) {
		this.dirigido = dirigido;
		this.grado = grado;
		this.nodos = new ArrayList<Nodo>();
		this.nodosVisitados = new LinkedList<Nodo>();
	...
	public void resolver(int idNodo) {
		...
		while (!this.nodosVisitados.isEmpty()) {		
			...
			Nodo nodoActual = this.nodosVisitados.poll();	
			...
			for(int j = 0; j < this.nodos.size() ; j++)
			{
				if (this.matriz[nodoActual.getId() -1 ][j] != Integer.MAX_VALUE && !this.nodos.get(j).getVisitado()) {
					...
					this.nodosVisitados.add(nodoNoVisitado);
				}
			}				
		} 	
	}
